...
function ajaxCore$1(options, callback) {
  ...
  function parseJsonMultipart(obj$$1, boundary) {
    var results = [];
    var blocks = obj$$1.split("--" + boundary);
      ...
      try {
        if (options.acceptJsonMultipart) {
          ...
          if (matches && matches[1] === 'multipart/mixed') {
            obj$$1 = parseJsonMultipart(obj$$1, matches[2]);
          } else {
...
function HttpPouch(opts, callback) {
  ...
  api.bulkGet = adapterFun('bulkGet', function (opts, callback) {
    ...
    function wrapCallbackForMultipart(cb) {
      return function (err, obj$$1, resp) {
        if (err || !resp || !resp.contentType) {
          cb(err, obj$$1, resp);
          return;
        }
        ...
        var results = obj$$1.map(function (item) {
          if (item.error) {
            return {
              "docs": [{"error": item}],
              "id": item["_id"]
